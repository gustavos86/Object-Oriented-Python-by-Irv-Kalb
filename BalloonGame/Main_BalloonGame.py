...
oScoreDisplay = pygwidgets.DisplayText(window,
                                       ...
                                       width=140,
                                       fontSize=24)
oStatusDisplay = pygwidgets.DisplayText(window,
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if playing:
            ...
            oScoreDisplay.setValue(f"Score: {str(theScore)}")
    ...
    if playing:
        ...
        nInitialBalloons = oBalloonMgr.getNumberOfInitialBalloons()
        oStatusDisplay.setValue(f"Popped: {nPopped}, Missed: {nMissed} Out of: {nInitialBalloons}")

        # if (nPopped + nMissed) == N_BALLONS:
        if not oBalloonMgr.getCountBalloonsInMotion():  # Zero Ballons left on the screen
            playing = False
            oStartButton.enable()
