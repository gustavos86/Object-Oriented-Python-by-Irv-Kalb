...
class SceneSplash(pyghelpers.Scene):
    ...
    def handleInputs(self, events, keyPressedList):        
        for event in events:
            if self.startButton.handleEvent(event):  # Clicked Start
                self.goToScene(SCENE_PLAY)
            elif self.quitButton.handleEvent(event):  # Clicked Quit
                self.quit()
            elif self.highScoresButton.handleEvent(event):  # Clicked High Scores
